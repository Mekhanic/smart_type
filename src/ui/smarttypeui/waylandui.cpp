...
void WaylandUI::flash(InputContext * /*inputContext*/, const std::string &correction) {
    if (inputWindow_) {
        if (is_debug_enabled()) {
            std::cerr << "[SmartTypeUI] correction-pulse started: word=\""
                      << correction << "\" duration=" << InputWindow::correctionPulseDurationMs_ << "ms" << std::endl;
        }
        inputWindow_->startCorrectionPulse();
    }
}

void WaylandUI::bounce(InputContext * /*inputContext*/, bool right) {
    if (inputWindow_) {
        inputWindow_->startBounce(right);
    }
}
