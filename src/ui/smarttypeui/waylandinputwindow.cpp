...
void WaylandInputWindow::setBlurManager(
    ...
    if (is_debug_enabled()) {
        std::cerr << "[SmartTypeUI] blur manager: "
                  << (blurManager_ ? "available (KWin blur protocol active)" : "NOT available")
                  << std::endl;
    }
    updateBlur();
}
...
void WaylandInputWindow::update(fcitx::InputContext *ic) {
    ...
    if (ic->frontendName() == "wayland_v2") {
        if (!panelSurfaceV2_ || ic != v2IC_.get()) {
            ...
            // ── SmartType surface diagnostic ──────────────────────────────────
            if (is_debug_enabled()) {
                std::cerr << "[SmartType] Native input-popup surface: "
                          << (panelSurfaceV2_ ? "created" : "FAILED") << " | "
                          << "Frontend: wayland_v2 | "
                          << "Program: " << ic->program() << "\n";
            }
            // ───────────────────────────────────────────────────────────────────
        ...
        if (!panelSurface_) {
            ...
            // ── SmartType surface diagnostic ──────────────────────────────────
            if (is_debug_enabled()) {
                std::cerr << "[SmartType] Native input-panel surface: "
                          << (panelSurface_ ? "created" : "FAILED") << " | "
                          << "Frontend: wayland (v1) | "
                          << "Program: " << ic->program() << "\n";
            }
            // ───────────────────────────────────────────────────────────────────
