...
void ClassicUI::reloadConfig() {
    // SmartType UI must not inherit the unrelated distro Classic UI theme.
    // That made fresh X11 installs render a plain white candidate rectangle.
    readAsIni(config_, "conf/smarttypeui.conf");
    reloadTheme();
}
...
void ClassicUI::reloadTheme() {
    ...
    // ── SmartTypeUI theme runtime diagnostics ────────────────────────────────
    {
        const auto &t = theme_;
        const auto &ip = *t.inputPanel;

        // Resolved theme name and whether it was loaded from user path
        std::string resolvedPath = "(no file found – fallback defaults used)";
        {
            namespace fs = std::filesystem;
            // Check user path first, then system
            const char *home = std::getenv("HOME");
            if (home) {
                fs::path userTheme = fs::path(home) /
                    ".local/share/fcitx5/themes" / t.name() / "theme.conf";
                if (fs::exists(userTheme))
                    resolvedPath = "user: " + userTheme.string();
            }
            if (resolvedPath.find("user:") == std::string::npos) {
                fs::path sysTheme =
                    "/usr/share/fcitx5/themes" / fs::path(t.name()) / "theme.conf";
                if (fs::exists(sysTheme))
                    resolvedPath = "system: " + sysTheme.string();
            }
        }

        // Blur manager: logged separately when WaylandInputWindow initialises
        // (see waylandinputwindow.cpp setBlurManager). We only note presence here.
        const bool blurAvailable = !uis_.empty(); // placeholder; actual blur state in WaylandInputWindow log
        (void)blurAvailable;

        auto colorStr = [](const Color &c) -> std::string {
            char buf[32];
            std::snprintf(buf, sizeof(buf), "rgba(%.0f,%.0f,%.0f,%.3f)",
                          c.redF() * 255, c.greenF() * 255,
                          c.blueF() * 255, c.alphaF());
            return buf;
        };

        if (is_debug_enabled()) {
            std::cerr << "[SmartTypeUI Theme] ──────────────────────────────────\n"
                      << "  theme name     : " << t.name() << "\n"
                      << "  resolved path  : " << resolvedPath << "\n"
                      << "  isSystemTheme  : " << t.isSystemTheme() << "\n"
                      << "  blur manager   : see WaylandInputWindow setBlurManager log\n"
                      << "  background color: " << colorStr(t.inputPanelBackground()) << "\n"
                      << "  normalColor    : " << colorStr(t.inputPanelText()) << "\n"
                      << "  highlightText  : " << colorStr(t.inputPanelHighlightText()) << "\n"
                      << "  highlightCandidateText: " << colorStr(t.inputPanelHighlightCandidateText()) << "\n"
                      << "  highlightBackground  : " << colorStr(t.inputPanelHighlight()) << "\n"
                      << "  highlightCandBg: " << colorStr(t.inputPanelHighlightCandidateBackground()) << "\n"
                      << "  pill image     : " << *ip.highlight->image << "\n"
                      << "  pill HighlightBackgroundColor (theme.conf): "
                      << (*ip.highlightBackgroundColor).toString() << "\n"
                      << "  accent color   : "
                      << (accentColor_ ? colorStr(*accentColor_) : "none") << "\n"
                      << "  candidateLabelColor: " << colorStr(t.inputPanelCandidateLabelText()) << "\n"
                      << "  draw order     : background → pill (first pass) → text (second pass)\n"
                      << "[SmartTypeUI Theme] ──────────────────────────────────" << std::endl;
        }
...
void ClassicUI::flash(InputContext *inputContext, const std::string &correction) {
    if (auto *uiPtr = findValue(uis_, inputContext->display())) {
        uiPtr->get()->flash(inputContext, correction);
    }
}

void ClassicUI::bounce(InputContext *inputContext, bool right) {
    if (auto *uiPtr = findValue(uis_, inputContext->display())) {
        uiPtr->get()->bounce(inputContext, right);
    }
}

} // namespace fcitx::classicui

FCITX_ADDON_FACTORY_V2(smarttypeui, fcitx::classicui::ClassicUIFactory);
