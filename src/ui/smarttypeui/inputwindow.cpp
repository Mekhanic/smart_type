...
void setupFontDescription(PangoFontDescription *fontDesc, double s = 1.0) {
    pango_font_description_set_family(fontDesc, "Noto Sans");
    pango_font_description_set_absolute_size(
        fontDesc, static_cast<int>(13.0 * s * PANGO_SCALE));
...
int MultilineLayout::width() const {
    ...
    for (size_t i = 0; i < lines_.size(); i++) {
        // Normal-weight width
        if (i < attrLists_.size() && attrLists_[i]) {
            pango_layout_set_attributes(lines_[i].get(), attrLists_[i].get());
        }
        int wNormal = 0;
        int h = 0;
        pango_layout_get_pixel_size(lines_[i].get(), &wNormal, &h);

        // Semibold-weight width (for layout-shift prevention)
        int wBold = wNormal;
        if (i < highlightAttrLists_.size() && highlightAttrLists_[i]) {
            pango_layout_set_attributes(lines_[i].get(),
                                        highlightAttrLists_[i].get());
            pango_layout_get_pixel_size(lines_[i].get(), &wBold, &h);
            // Restore normal attrs
            if (i < attrLists_.size() && attrLists_[i]) {
                pango_layout_set_attributes(lines_[i].get(),
                                            attrLists_[i].get());
            }
        ...
        width = std::max(width, std::max(wNormal, wBold));
    }
    return width;
}
...
void MultilineLayout::render(cairo_t *cr, int x, int y, bool highlight) {
    ...
    if (!lines_.empty()) {
        ...
        if (context) {
            const auto *fontDescription = pango_layout_get_font_description(lines_[0].get());
            if (!fontDescription) {
                fontDescription = pango_context_get_font_description(context);
            }
            if (fontDescription) {
                auto *metrics = pango_context_get_metrics(context, fontDescription, pango_context_get_language(context));
                int descent = pango_font_metrics_get_descent(metrics);
                pango_font_metrics_unref(metrics);
                shiftY = -PANGO_PIXELS(descent) / 2;
            }
        }
    }

    int allocatedW = width();
    ...
    for (size_t i = 0; i < lines_.size(); i++) {
        ...
        int lineW = 0;
        int lineH = 0;
        pango_layout_get_pixel_size(lines_[i].get(), &lineW, &lineH);
        int offsetX = (allocatedW - lineW) / 2;

        renderLayout(cr, lines_[i].get(), x + offsetX, y + shiftY);
        y += lineHeight;
    }
}
...
void InputWindow::insertAttr(PangoAttrList *attrList, TextFormatFlags format,
                             ...
                             TextType type) const {
    if (highlight && type == TextType::Regular) {
        auto *attr = pango_attr_weight_new(PANGO_WEIGHT_SEMIBOLD);
    ...
    // Global letter spacing of -0.1px
    {
        auto *attr = pango_attr_letter_spacing_new(static_cast<int>(-0.1 * PANGO_SCALE));
        attr->start_index = start;
        attr->end_index = end;
        pango_attr_list_insert(attrList, attr);
    }
...
std::pair<int, int> InputWindow::update(InputContext *inputContext) {
    ...
    // ST-042: dual-IM SmartType uses Label=RU/EN. Fcitx compact
    // ShowInputMethodInformation paints that into auxUp and the upper row
    // grows the candidate panel. Drop pure layout-label toasts so geometry
    // stays candidate-only. Tray / Plasma indicators still show the layout.
    {
        const auto auxStr = auxUp.toString();
        if (auxStr == "RU" || auxStr == "EN" || auxStr == "SmartType Russian" ||
            auxStr == "SmartType English" || auxStr == "SmartType Русский" ||
            auxStr == "SmartType Английский") {
            auxUp = Text();
        }
    }
...
std::pair<unsigned int, unsigned int> InputWindow::sizeHint() {
    ...
    const double s = *parent_->config().candidatePanelScale / 100.0;
    auto *fontDesc =
        pango_font_description_from_string(parent_->config().font->c_str());
    setupFontDescription(fontDesc, s);
...
void InputWindow::paint(cairo_t *cr, unsigned int width, unsigned int height,
    ...
    // First pass: resolve layout, target highlight coordinates, and regions
    for (size_t i = 0; i < nCandidates_; i++) {
        float candidateLeft =
        ...
        if (highlightIndex >= 0 && i == static_cast<size_t>(highlightIndex)) {
            targetX = candidateLeft - *highlightMargin.marginLeft;
            targetY = candidateTop - *highlightMargin.marginTop;
            targetW = highlightWidth + *highlightMargin.marginLeft + *highlightMargin.marginRight;
            targetH = candidateHeight + *highlightMargin.marginTop + *highlightMargin.marginBottom;
            // A one-glyph candidate can be narrower than the pill height. Cairo's
            // capsule arcs then cross and produce a pointed/self-intersecting path.
            // Expand symmetrically so every highlight remains a true capsule.
            if (targetW < targetH) {
                targetX -= (targetH - targetW) / 2.0;
                targetW = targetH;
            }
            hasTarget = true;
        }
    ...
    double drawY = targetY;
    double drawW = targetW;
    double drawH = targetH;

    if (hasTarget) {
        if (std::abs(targetX - targetX_) > 0.01 || std::abs(targetY - targetY_) > 0.01 ||
            std::abs(targetW - targetW_) > 0.01 || std::abs(targetH - targetH_) > 0.01) {
            startAnimation(targetX, targetY, targetW, targetH);
        }

        if (animating_) {
            auto nowTime = std::chrono::steady_clock::now();
            double t = std::chrono::duration<double>(nowTime - animStartTime_).count() / 0.150; // 150ms
            if (t >= 1.0) {
                t = 1.0;
                animating_ = false;
            }
            double progress = easeOutCubic(t);
            currentX_ = startX_ + (targetX_ - startX_) * progress;
            currentY_ = startY_ + (targetY_ - startY_) * progress;
            currentW_ = startW_ + (targetW_ - startW_) * progress;
            currentH_ = startH_ + (targetH_ - startH_) * progress;
        ...
        // Apply edge bounce feedback if active
        if (bouncing_) {
            auto nowTime = std::chrono::steady_clock::now();
            double t = std::chrono::duration<double>(nowTime - bounceStartTime_).count() / 0.120; // 120ms duration
            if (t >= 1.0) {
                bouncing_ = false;
            } else {
                animating_ = true; // keep repaint loop active
                double factor = 1.0 - t;
                const double s = *parent_->config().candidatePanelScale / 100.0;
                double shift = 2.5 * s * factor;
                if (!bounceRight_) {
                    drawX -= shift;
                    drawW += shift;
                } else {
        ...
        // Draw animated selection pill via direct Cairo rounded-rect.
        // This guarantees a perfect capsule at any width regardless of
        // highlight SVG margins — the radius is always half the height.
        if (drawW > 0 && drawH > 0) {
            cairo_save(cr);

            // Clip to the inner capsule area defined by Background/Margin
            // so the pill never overflows the panel's rounded edges.
            const auto &bgMargin = *theme.inputPanel->background->margin;
            double logicalWidth = width / scale;
            double logicalHeight = height / scale;
            double clipLeft   = *bgMargin.marginLeft;
            double clipTop    = *bgMargin.marginTop;
            double clipRight  = logicalWidth - *bgMargin.marginRight;
            double clipBottom = logicalHeight - *bgMargin.marginBottom;
            double clipH      = clipBottom - clipTop;
            double clipR      = clipH / 2.0;

            cairo_new_sub_path(cr);
            cairo_arc(cr, clipRight - clipR, clipTop    + clipR, clipR, -M_PI_2, 0);
            cairo_arc(cr, clipRight - clipR, clipBottom - clipR, clipR, 0,       M_PI_2);
            cairo_arc(cr, clipLeft  + clipR, clipBottom - clipR, clipR, M_PI_2,  M_PI);
            cairo_arc(cr, clipLeft  + clipR, clipTop    + clipR, clipR, M_PI,   -M_PI_2);
            cairo_close_path(cr);
            cairo_clip(cr);
            ...
            double pw = drawW;
            double ph = drawH;
            double pr = ph / 2.0;  // always a perfect capsule

            cairo_new_sub_path(cr);
            cairo_arc(cr, px + pw - pr, py + pr, pr, -M_PI_2, 0);
            cairo_arc(cr, px + pw - pr, py + ph - pr, pr, 0, M_PI_2);
            cairo_arc(cr, px + pr, py + ph - pr, pr, M_PI_2, M_PI);
            cairo_arc(cr, px + pr, py + pr, pr, M_PI, -M_PI_2);
            cairo_close_path(cr);
            cairo_set_source_rgba(cr, 1.0, 1.0, 1.0, 0.12);
            cairo_fill(cr);

            // Top inset highlight — white 10%, arcs only along the top half
            cairo_new_sub_path(cr);
            cairo_arc(cr, px + pr, py + pr, pr, M_PI, -M_PI_2);      // left arc top half
            cairo_arc(cr, px + pw - pr, py + pr, pr, -M_PI_2, 0);    // right arc top half
    ...
    // Second pass: draw the static candidate text layouts
    for (size_t i = 0; i < nCandidates_; i++) {
        float labelLeft =
        ...
        const int highlightIndex = highlight();
        bool isHighlightedCandidate = (highlightIndex >= 0 && i == static_cast<size_t>(highlightIndex));
        ...
        if (candidateLayouts_[i].text.characterCount()) {
            candidateLayouts_[i].text.render(cr, textLeft, textTop, isHighlightedCandidate);
        }
    ...
    // ── Correction pulse: horizontal light sweep across the panel ─────────
    if (correctionPulse_ && width > 0 && height > 0) {
        auto elapsed = static_cast<double>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - correctionPulseStart_).count());
        const double duration = static_cast<double>(correctionPulseDurationMs_);
        double t = std::clamp(elapsed / duration, 0.0, 1.0);

        // Sweep position: a bright band moves from left to right then fades
        // Band width = 40% of panel width; peak alpha = 0.30 (subtle)
        const double bandW = static_cast<double>(width) * 0.40;
        // Centre moves from -bandW/2 to width + bandW/2 over the duration
        const double cx = -bandW * 0.5 + (static_cast<double>(width) + bandW) * t;

        cairo_save(cr);
        auto *pat = cairo_pattern_create_linear(cx - bandW * 0.5, 0,
                                                cx + bandW * 0.5, 0);
        cairo_pattern_add_color_stop_rgba(pat, 0.0, 1, 1, 1, 0.0);
        cairo_pattern_add_color_stop_rgba(pat, 0.5, 1, 1, 1, 0.30);
        cairo_pattern_add_color_stop_rgba(pat, 1.0, 1, 1, 1, 0.0);
        cairo_set_source(cr, pat);
        cairo_pattern_destroy(pat);
        cairo_set_operator(cr, CAIRO_OPERATOR_OVER);
        cairo_rectangle(cr, 0, 0, static_cast<double>(width), static_cast<double>(height));
        cairo_fill(cr);
        cairo_restore(cr);
    }
    // ─────────────────────────────────────────────────────────────────────────
...
bool InputWindow::hover(int x, int y) {
    ...
    const bool pointerLeftWindow = x < 0 || y < 0;
    ...
    if (!prevHovered) {
        ...
        if (!nextHovered && !pointerLeftWindow) {
    ...
    // Keep the last hovered candidate while crossing the visual gap between
    // candidates. Falling back to candidateIndex_ here made the pill jump to
    // the keyboard-selected item (usually index 0) for a single pointer frame.
...
void InputWindow::updateYogaLayout() {
    ...
    // Configure root node — all geometry values scaled by candidatePanelScale
    const double s = *parent_->config().candidatePanelScale / 100.0;
    const auto &bgMargin = *theme.inputPanel->background->margin;
    YGNodeStyleSetPadding(rootNode_.get(), YGEdgeLeft,
        (*margin.marginLeft  + *bgMargin.marginLeft)  * s);
    YGNodeStyleSetPadding(rootNode_.get(), YGEdgeRight,
        (*margin.marginRight + *bgMargin.marginRight) * s);
    YGNodeStyleSetPadding(rootNode_.get(), YGEdgeTop,
        (*margin.marginTop   + *bgMargin.marginTop)   * s);
    YGNodeStyleSetPadding(rootNode_.get(), YGEdgeBottom,
        (*margin.marginBottom + *bgMargin.marginBottom) * s);
    YGNodeStyleSetMaxWidth(rootNode_.get(), 520.0f * s);
...
void InputWindow::startAnimation(double targetX, double targetY, double targetW, double targetH) {
    if (!animating_) {
        startX_ = currentX_ != -9999.0 ? currentX_ : targetX;
        startY_ = currentY_ != -9999.0 ? currentY_ : targetY;
        startW_ = currentW_ != -9999.0 ? currentW_ : targetW;
        startH_ = currentH_ != -9999.0 ? currentH_ : targetH;
    } else {
        startX_ = currentX_;
        startY_ = currentY_;
        startW_ = currentW_;
...
void InputWindow::scheduleNextFrame() {
    if (!animating_) {
        animationTimer_.reset();
        return;
    }
    auto &loop = parent_->instance()->eventLoop();
    animationTimer_ = loop.addTimeEvent(
        CLOCK_MONOTONIC, fcitx::now(CLOCK_MONOTONIC) + 16000, 1,
...
void InputWindow::startCorrectionPulse() {
    // Disabled per user request (not approved in final design)
    return;
}

void InputWindow::startBounce(bool right) {
    bouncing_ = true;
    bounceRight_ = right;
    bounceStartTime_ = std::chrono::steady_clock::now();
    animating_ = true;
    if (!animationTimer_) {
        scheduleNextFrame();
    }
